...
1. Install MATLAB Runtime and the FlyerScan CT executable by double clicking on **MyAppInstaller.exe** (in releases).
2. The FlyerScan CT app my be executed by double clicking the **flyerscan.exe** executable file (generated by installer), or calling the function from the DOS command window, or by using a batch file (see **main.bat**).
